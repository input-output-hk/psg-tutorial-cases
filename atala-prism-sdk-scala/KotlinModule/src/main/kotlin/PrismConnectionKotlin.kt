...
object PrismConnectionKotlin {
    @kotlin.ExperimentalUnsignedTypes
    fun createDiD(environment: String,nameRegistered:String){
        var connector = ProtoClientUtils.connectorClient(environment, 50051)
        var node = ProtoClientUtils.nodeClient(environment, 50053)
    ...
    """.trimIndent()
        ...
        sleep(10* 1000*60)
        connector = ProtoClientUtils.connectorClient(environment, 50051)
        node = ProtoClientUtils.nodeClient(environment, 50053)
        ...
        val verifierReceivedCredentialIssuanceKeyIdKotlinString:kotlin.String = verifierReceivedCredentialIssuanceKeyId


        println("FIRST WAKE UP")

        val verifierReceivedCredentialIssuerKey = verifierReceivedCredentialIssuerDIDDocument.findPublicKey(verifierReceivedCredentialIssuanceKeyIdKotlinString)
        ...
        val issuerRevokeCredentialOperation = ProtoUtils.revokeCredentialsOperation(
                batchOperationHash = SHA256Digest.compute(issueCredentialOperation.encodeToByteArray().asList()),
    ...
    """.trimIndent()
        ...
        val verifierReceivedJsonCredentialByte = verifierReceivedJsonCredential.hash().value.map{ it.toByte() }

        val verifierReceivedCredentialRevocationTime2 = runBlocking {
            node.GetCredentialRevocationTime(
                    GetCredentialRevocationTimeRequest(
                            batchId = SHA256Digest.fromHex(verifierReceivedCredentialBatchId.id).hexValue(),
                            credentialHash = pbandk.ByteArr(verifierReceivedJsonCredentialByte.toByteArray())
                    )
            ).revocationLedgerData?.timestampInfo?.toTimestampInfoModel()
        }


        println("THRITH WAKE UP")
